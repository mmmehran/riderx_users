...
import {ArrowLeft2, KeyboardIcon} from '../../../assets/svg/index';
...
const ResetPassword = route => {
  ...
  const onSubmit = async value => {
    setLoading(true);
    Keyboard.dismiss();
    if (/^[^@\s]+@bb\.com$/i.test(route?.route?.params?.email)) setConfigTest();
    else setConfig();
    await new Promise(r => setTimeout(r, 300));
    const response = await sendData(
      urls.UPDATEPASSWORD,
      {
        code: route?.route?.params?.code,
          ...
          <Form
            ...
            {({values}) => (
              <>
                <Input
                  name="password"
                  inputName={t('enterYourNewPassword')}
                ...
                <Input
                  name="rePassword"
                  inputName={t('enterYourConfirmPassword')}
                  input={{textAlign: 'left'}}
                  password
                  autoCapitalize="none"
