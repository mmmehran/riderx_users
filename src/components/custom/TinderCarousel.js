...
/* ---------------- CardLayer: hooks live here (not inside a loop) ---------------- */
function CardLayer({
  item,
  idx,
  layer,
  ...
  const body = (
    <Animated.View
      style={[
        styles.card,
        { width: cardWidth, height: cardHeight, zIndex: 100 - layer },
        style,
      ]}
    >
...
export default function TinderCarousel({
  ...
  // force React re-render when we advance
  ...
  // Plain JS function (NOT a worklet)
  const advance = useCallback((dir) => {
    index.value = clampIndex(index.value + 1); // ok to set shared values from JS
    tx.value = 0;
    ty.value = 0;
    rot.value = 0;
    if (onIndexChange) onIndexChange(index.value);
    bump(); // rebuild stack list
  }, [clampIndex, index, tx, ty, rot, onIndexChange]);

  // JS handler to run after swipe finishes; receives the numeric index from UI
  const handleSwipeJS = useCallback(
    (i, dir) => {
      const item = data[i];
      if (item && onSwipe) onSwipe(item, dir);
      advance(dir);
    },
    ...
    .onEnd(() => {
      ...
      if (absX > SWIPE_THRESHOLD) {
        ...
        tx.value = withTiming(finalX, { duration: 180 }, (finished) => {
          if (!finished) return;
          // Call back to JS safely with the numeric index
          runOnJS(handleSwipeJS)(index.value, dir);
        });
  ...
  // compute next few cards from current index (React side)
